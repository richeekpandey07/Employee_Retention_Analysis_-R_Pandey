...
st.title("🤝 Employee Retention Analysis")
st.write("Employee Retention Prediction using Logistic Regression")
...
try:
    ...
    st.error("Dataset file 'HR_comma_sep (2).csv' not found.")
    st.stop()
...
# Average values
# ----------------------------
st.subheader("Average Values by Employee Retention")
...
pd.crosstab(df.salary, df.left).plot(
    kind="bar",
    ax=ax1
)
...
ax2.set_title("Department vs Employee Retention")
